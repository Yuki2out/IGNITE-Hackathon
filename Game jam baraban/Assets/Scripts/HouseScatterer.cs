...

{
    ...
    public int houseLayerIndex; // Set this to the ID of your "Houses" layer
        ...
        while (placedCount < numberOfHouses && attempts < maxAttempts)
        {
...

            {
                ...
                if (!IsSpotTaken(hit.point))
                {
                    ...
                    Debug.Log("Children count: " + newHouse.transform.childCount);
                   
                    // FORCE the layer so the next check sees it

                    SetLayerRecursively(newHouse, houseLayerIndex);
